...
public class AdminController {
    ...
    @FXML
    private TextField create_operator_fname;
    @FXML
    private TextField create_operator_lname;
    @FXML
    private TextField create_operator_email;
    @FXML
    private TextField create_operator_pass;
    @FXML
    private TextField create_operator_phone;
    @FXML
    ...
    public boolean createOperator(){
        SignUpDTO signUpDTO = new SignUpDTO(create_operator_fname.getText(), create_operator_lname.getText(), create_operator_email.getText(), create_operator_pass.getText(), create_operator_phone.getText());
    ...
    public void initialize() {
        for (BookCovers bc : commonAdminOperatorFunctionalities.retrieveBookCovers(adminService)) {
            add_book_cover.getItems().add(bc.getCoverName());
        }

        search_book_state.getItems().add("All");
        for (BookState bs : commonAdminOperatorFunctionalities.retrieveBookState(adminService)) {
            search_book_state.getItems().add(bs.getStateName());
        }

        search_book_genre.getItems().add("All");
        for (Genre g : commonAdminOperatorFunctionalities.retrieveBookGenre(adminService)) {
        ...
        create_btn.setOnAction(event -> {
            ...
            if (!nameError.errors(fname.getText()).equals("Name errors:")) {
                ...
                boolean isCreated = commonAdminOperatorFunctionalities.createReader(adminService, fname, lname, email, pass, phone);
        ...
        add_book_btn.setOnAction(event -> {
            if(commonAdminOperatorFunctionalities.checkNullValuesAddBook(add_book_ID, add_book_isbn, add_book_issue_date, add_book_genre, add_book_cover, add_book_publisher, add_book_title, add_book_author)) {
                ...
                if (commonAdminOperatorFunctionalities.addBook(adminService, add_book_ID, add_book_isbn, add_book_issue_date, add_book_genre, add_book_cover, add_book_publisher, add_book_title, add_book_author )) {
        ...
        lend_for_reading_room_btn.setOnAction(event -> {
            ...
            commonAdminOperatorFunctionalities.lendBook(adminService, 2L, currentUser, chosen_books_for_rent, lend_rd_id);
        ...
        lend_for_home_btn.setOnAction(event -> {
            ...
            if(containsArchivedBook) {
                ...
                boolean successfulLend = commonAdminOperatorFunctionalities.lendBook(adminService, 1L, currentUser, chosen_books_for_rent, lend_rd_id);
                if (successfulLend) {
        ...
        return_btn.setOnAction(event -> {
            if (!return_table_view.getSelectionModel().isEmpty())
                commonAdminOperatorFunctionalities.updateAfterReturn(commonAdminOperatorFunctionalities.returnBooks(adminService, currentUser, return_table_view), return_table_view);
        });

        return_extend_btn.setOnAction(event -> {
            commonAdminOperatorFunctionalities.updateAfterExtendDueDate(commonAdminOperatorFunctionalities.extendDueDate(adminService, return_table_view), return_table_view);
        });
        ...
        create_operator_btn.setOnAction(event -> {
            ...
            Error nameError = new NotNullErrorDecorator(new OnlyCharErrorDecorator(new LongLengthErrorDecorator(new NameError())));
            Error phoneError = new NotNullErrorDecorator(new OnlyNumbersErrorDecorator(new PhoneError()));
            if (!nameError.errors(create_operator_fname.getText()).equals("Name errors:")) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText(nameError.errors(create_operator_fname.getText()));
                alert.show();
            } else if (!nameError.errors(create_operator_lname.getText()).equals("Name errors:")) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText(nameError.errors(create_operator_lname.getText()));
                alert.show();
            } else if(emailError.errors(create_operator_email.getText()) != null) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText(emailError.errors(create_operator_email.getText()));
                alert.show();
            } else if(!passError.errors(create_operator_pass.getText()).equals("Password errors:")) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText(passError.errors(create_operator_pass.getText()));
                alert.show();
            }else if (!phoneError.errors(create_operator_phone.getText()).equals("Phone errors:")) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText(phoneError.errors(create_operator_phone.getText()));
                alert.show();
            } else {
                logger.info(currentUser.getUserId().toString() + " attempting to create a new Reader");
                boolean isCreated = createOperator();
                commonAdminOperatorFunctionalities.nullifyCreateReaderFields(create_operator_fname, create_operator_lname, create_operator_email, create_operator_pass, create_operator_phone);
                Alert alert;
                if (isCreated) {
                    alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setContentText("Successfully created reader");
